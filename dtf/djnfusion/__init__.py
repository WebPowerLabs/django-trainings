...
def create_or_update_user(user=None, user_id=None):
    ...
    if len(results) == 0:
        ...
        return_result = create_or_update_user(user=user)
    else:
...
def update_user(infusionsoft_user_id, user=None, user_id=None):
    ...
    user = _user_or_id(user, user_id)
    # set the djnfusion_id field of user if it doesn't exist
    djnfusion_id = getattr(user, settings.USER_ID_FIELD_NAME[1:])
    if not djnfusion_id:
        setattr(user, settings.USER_ID_FIELD_NAME[1:], infusionsoft_user_id)
        user.save()
